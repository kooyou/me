...
%%获取可连接的游戏列表
get_game_connect_nodes() ->
	MS= ets:fun2ms(fun(Node) when Node#node.id > 10 -> Node end),
	ets:select(?ETS_NODE,MS).
...
init([Ip, Port, Sid]) ->
    ...
    add_node([State#state.ip, State#state.port, State#state.id, State#state.node,State#state.cookie]),
    %% 获取并通知当前所有线路
...
%%加入服务器集群
add_node([Ip, Port, Sid, Node,Cookie]) ->
	ets:insert(?ETS_NODE, #node{id = Sid,ip = Ip,node = Node,cookie = Cookie,port = Port}),
    db:execute(io_lib:format(<<"replace into `node` (id, ip, port, node, cookie) values(~p, '~s', ~p, '~s', '~s')">>,[Sid, Ip, Port, Node, Cookie])).

%%退出服务器集群
del_node(Sid) ->
	ets:delete(?ETS_NODE,Sid),
            ...
            F = fun([Id, Ip, Port, Node, Cookie, S]) ->
                    ...
                    case Id /= State#state.id of  % 自己不写入和不通知
                        true ->
                            ...
                            %        %% 集群设置
                            %        if
                            %            State#state.id =:= 0 orelse Id =:= 0 ->
                            %                erlang:set_cookie(Node1, Cookie1);
                            %            true ->
                            %                skip
                            %        end
                            %end,
